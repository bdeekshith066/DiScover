...
// Send dynamic pre-trip notification
router.post('/send/:tripId', async (req, res) => {
  ...
  try {
    ...
    // Send the dynamic pre-trip notification
    await preTripNotification.save();

    res.status(200).json({ message: 'Dynamic pre-trip notification sent successfully' });
  } catch (error) {
